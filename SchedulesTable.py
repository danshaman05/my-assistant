...
class SchedulesTable:
    def __init__(self, table_id:str, title:str, content: element.Tag):
        ...
        self.applicable_days: list[constants.DayTypes] = self.get_applicable_days()

    def get_applicable_days(self):
        result_list = []
        title_lst = [type_raw.lower() for type_raw in self.title.split(', ')]
        if 'pracovné dni' in title_lst:
            result_list.append(constants.DayTypes.WORKING_DAY)
        if 'voľné dni' in title_lst:
            result_list.append(constants.DayTypes.FREE_DAY)
        if 'školský rok' in title_lst:
            result_list.append(constants.DayTypes.SCHOOL_DAY)
        if 'školské prázdniny' in title_lst:
            result_list.append(constants.DayTypes.SCHOOL_HOLIDAY_DAY)
        return result_list
